...
def interp_location_landfrac(storm_df,landfrac_points,landfrac_values,frequency,interval):
    ...
    temp_df['ENUM'] = temp_df['ENUM'].values[0]
    temp_df         = temp_df.set_index(['ENUM'],append=True)
...
def create_storm_list(track_data,landfrac_points,landfrac_values,frequency,interval,calendar):
    ...
    for sid in np.unique(track_data.index.get_level_values('SID'))[::]:   # Loop over SID
        storm_sid        = track_data.xs(sid,level='SID').copy()  # Select the storm whose id is sid
